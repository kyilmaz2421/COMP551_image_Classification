...
class classifier():
    ...
    def search(self, X, Y, validation_size, tolerance):
        ...
        clfs = {} 
        self.recurrence(X, Y, validation_size, {}, self.params,
                        keys, 0, key_len, clfs, fit_count, tolerance)
    ...
    def recurrence(self, X, Y, validation_size, param, grid,
                   keys, depth, max_depth, clfs, task_count, tolerance):
                   
        if depth == max_depth:
            if tolerance!=-1: 
                score = self.iterative_search(X, Y, param,
                                              validation_size,tolerance)
            ...
            if clfs.get(score[0]): 
                ...
                clfs[score[0]] = param
            print(f"Score {score[0]} for Param: {param} - (task: {len(clfs.keys())}/{task_count})")
        else:
    ...
    def train_model(self, X, Y, validation_size,
        ...
        if model == None:
            clf = ANN(x_train, y_train, params["layers"],
                      params["batch"], params["activation"])
        else: 
            clf = model
            
        clf.fit(params["alpha"], params["epochs"],
    ...
    def iterative_search(self, X, Y, params,
                         validation_size, tolerance):
        ...
        while improvement or tolerance>0: # while improving
            ...
            if not improvement: 
              ...
              if tolerance < 3 and params["alpha"] < 0.01: # protect against local minima
                ...
                clf.mini_batch(np.split(x_train, 
                               params["batch"]),np.split(y_train, params["batch"]), 
                               params["alpha"],
                               params["regularization"],
                               params['momentum'])
            else: 
    ...
    def plot_learning_curve(self, cv_scores, test_scores,
                            train_sizes, xlabel = "Training examples"):
        plt.figure()
        plt.title("title")
        plt.xlabel(xlabel)
        plt.ylabel("Score")
    ...
    def learning_curve(self, X, Y,
        ...
        for t in train_sizes:
            ...
            (cv, test) = self.train_model(split_x, split_y, 
                                          validation_size, self.best_param, False)
            cv_scores.append(cv)
...
class ANN():
    def __init__(self, X, Y, hidden_layers, batch, activation):
        ...
        self.w = [np.random.randn(len(X[0]), hidden_layers[0]+1) * self.scale_weights(len(X[0]))] 
        self.velocity = [np.zeros((len(X[0]), hidden_layers[0]+1))]
        self.w[-1][:,:1] = np.random.uniform(low=-1e-5,
                                            high=1e-5,
                                            size=(len(self.w[-1][:,:1]),1)) * self.scale_weights(len(X[0]))
        
        for i in range(len(hidden_layers)-1): # size of w is L -1
            self.w.append(np.random.randn(hidden_layers[i]+1,
                          hidden_layers[i+1]+1) * self.scale_weights(hidden_layers[i]))

            self.w[-1][:,:1] = np.random.uniform(low=-1e-5,
                                                high=1e-5,
                                                size=(len(self.w[-1][:,:1]),1)) *self.scale_weights(hidden_layers[i])
            
            self.velocity.append(np.zeros((hidden_layers[i]+1, hidden_layers[i+1]+1)))
        
        self.w.append(np.random.randn(hidden_layers[-1]+1,
                                      len(Y[0])) * self.scale_weights(hidden_layers[-1]) )
        
        self.velocity.append(np.zeros((hidden_layers[-1]+1, len(Y[0]))))
        self.w[-1][:,:1] = np.random.uniform(low=-1e-5,
                                            high=1e-5,
                                            size=(len(self.w[-1][:,:1]),1)) * self.scale_weights(len(Y[0]))
    ...
    def fit(self, alpha, epochs, regularization, momentum):
        for i in range(epochs):
            ...
            self.mini_batch(np.split(self.data[:,0:-10], self.batch),
                            np.split(self.data[:,-10:], self.batch),
                            alpha, regularization, momentum)
    ...
    def mini_batch(self, X, Y, alpha, regularization, momentum):
        for i in range(self.batch):
            ...
            self.back_prop(X[i],Y[i],act_units,
                           alpha, regularization, momentum)
    ...
    def back_prop(self,X,Y,act_units,alpha,regularization,momentum):
        ...
        for l in range(2, self.L):
            dz = np.dot(dz, 
                        self.w[-l+1].T) * self.deriv(self.activation,act_units[-l])
